...
test('arduino verify should compile on arduino 1.8.2', async t => {
  const arduinoObj = arduino({tag: 'verify', version: '1.8.2'});
  await pify(arduinoObj.load)();
  const err = await t.throws(pify(arduinoObj.run)(['--verify', fixture('invalid/invalid.ino')]));
  t.is(err.failed, true);
  const out = await pify(arduinoObj.run)(['--verify', fixture('empty/empty.ino')]);
...
test('arduino verify should compile on arduino 1.8.1', async t => {
  const arduinoObj = arduino({tag: 'verify', version: '1.8.1'});
  await pify(arduinoObj.load)();
  const err = await t.throws(pify(arduinoObj.run)(['--verify', fixture('invalid/invalid.ino')]));
  t.is(err.failed, true);
  const out = await pify(arduinoObj.run)(['--verify', fixture('empty/empty.ino')]);
